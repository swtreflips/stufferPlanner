...
export default function ContainerCard({ container }: Props) {
  ...
  const metrics = useMemo(() => {
    ...
    return { lines: allocations.length, totalQty, totalCbm }
  ...
  const showDropAffordance = active !== null && isMasterItemDragData(activeData)
  ...
  const dropStateClass = !showDropAffordance
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pb-2 text-[10px] font-mono uppercase tracking-widest text-navy-400">
        by {container.createdBy}
      </div>
      ...
      <div className="px-4 py-3 border-t border-navy-100 min-h-[3.5rem]">
        {showDropAffordance && !destinationMatches ? (
          ...
          <div className="text-xs italic text-navy-400">No allocations yet</div>
        ) : (
              ...
              return (
                <li key={a.id}>
                  <AllocationCard
                    ...
                    masterItem={item}
                    onClick={() =>
                      openAllocationDialog({ kind: 'edit', allocationId: a.id })
                    }
                  />
                </li>
              )
      ...
      <footer className="flex justify-end px-4 py-2 border-t border-navy-100">
        <button
          type="button"
...
function StatusBadge({ status }: { status: Container['status'] }) {
  if (status === 'committed') {
    return (
      <span className="text-[10px] font-mono uppercase tracking-widest px-2 py-0.5 rounded bg-teal-accent/10 text-teal-accent border border-teal-accent/30">
        committed
      </span>
    )
  }
