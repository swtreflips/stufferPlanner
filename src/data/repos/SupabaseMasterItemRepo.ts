...
import type { CbmFigures, MasterItemRepo } from './types'
import type { CbmSource } from '../../types/masterItem'
...
interface Row {
  ...
  // The RAW inputs — nullable, and exactly one of them is set. Which one is which is the
  // difference between a measurement and a calculation, so it is carried through to the grid.
  cbm_per_case: number | null
  cbm_total: number | null
  // The RESOLVED pair, generated by Postgres from whichever raw figure is present.
...
const cbmSourceOf = (r: {
  ...
  r.cbm_per_case !== null ? 'per_case' : r.cbm_total !== null ? 'total' : null

/*
  One write path for both CBM figures.

  Module-level rather than a method, for the same reason as `rpc()` below: `this` only binds
  while a method is called off its object, so destructuring the repo would break it at runtime
  with nothing at compile time to catch it.
*/
async function writeCbm(
  id: string,
  patch: { cbm_per_case: number | null; cbm_total: number | null },
  what: string,
): Promise<CbmFigures> {
  const { data, error } = await supabase
    ...
    .update(patch)
    .eq('id', id)
    // Returning the row is what makes the sibling column update without a reload: the generated
    // values arrive already computed, in the same round trip as the save.
    .select('cbm_per_case, cbm_total, cbm_per_case_eff, cbm_total_eff')
    .single()
  if (error) throw new Error(`Failed to update ${what}: ${error.message}`)

  const r = data as unknown as Row
  return {
    cbmPerCase: r.cbm_per_case_eff ?? 0,
    cbmTotal: r.cbm_total_eff ?? 0,
    cbmSource: cbmSourceOf(r),
  }
}
...
export function createSupabaseMasterItemRepo(): MasterItemRepo {
  return {
    ...
    /*
      CBM — SUPPLY ONE FIGURE, THE OTHER IS DERIVED.

      `cbm_per_case_eff` and `cbm_total_eff` are GENERATED columns, writable by nobody. They
      resolve whichever raw figure was supplied and compute the other against quantity_available.
      Crucially the resolution is ASYMMETRIC: cbm_per_case wins when both are set.

      So each write CLEARS ITS SIBLING. Without that, typing a Total CBM on a row that already
      had a per-case figure would be written faithfully and change nothing on screen — the app
      appearing to ignore what you typed, which is worse than the field being read-only.

      It is also what makes the stored figure meaningful. When the weekly ERP sync revises
      quantity_available, a stored per-case holds and the total moves; a stored total holds and
      the per-case moves. Which one somebody typed is a lasting property of the row.

      Both return the resolved pair straight from the database, so the client never multiplies or
      divides anything and cannot disagree with the generated columns — nor get the divisor wrong
      (quantity_available, not quantity).
    */
    async updateCbmPerCase(id, value) {
      return writeCbm(id, { cbm_per_case: value, cbm_total: null }, 'CBM per case')
    },

    async updateCbmTotal(id, value) {
      return writeCbm(id, { cbm_total: value, cbm_per_case: null }, 'total CBM')
    },
